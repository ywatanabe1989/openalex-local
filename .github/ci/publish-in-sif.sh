...
# (project=openalex-local, owner=ywatanabe1989, repo=openalex-local,
...
TMPDIR="/tmp/publish-openalex_local-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
export TMPDIR
rm -rf "$TMPDIR"
...
echo "PUBLISH-OK: openalex-local dist/* uploaded to PyPI via manual OIDC trusted publishing"
