...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT openalex-local's —
...
# so only openalex-local's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-openalex_local-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# openalex-local's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install openalex-local + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors openalex-local's historical bare-uv/pip workflow so a
...
# openalex-local's suite is ~2460 tests; single-process it overran the job's old
...
# openalex-local style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: openalex-local-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and openalex-local's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/openalex_local --cov-report=xml --cov-report=term \
    -p no:cacheprovider
