...
2. Convert the [JSON Lines](https://jsonlines.org/) files you [downloaded](02_download.md) to [CSV](https://en.wikipedia.org/wiki/Comma-separated_values) files that can be read by the database application. We'll flatten them to fit a [hierarchical database model](https://en.wikipedia.org/wiki/Hierarchical_database_model).
...
> **Note**: SQL in this section isn't anything additional you need to run. It's part of the schema we already defined in the file above.
...
> **Warning**: This script assumes your downloaded snapshot is in `openalex-snapshot` and you've made a directory `csv-files` to hold the CSV files.
>
> Edit `SNAPSHOT_DIR` and `CSV_DIR` at the top of the script to read or write the files somewhere else.

> **Note**: This script has only been tested using python 3.9.5.
...
> **Note**: This script is slow. Exactly how slow depends on the machine you run it on, but think hours, not minutes.
>
